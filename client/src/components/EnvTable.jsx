...
import { Eye, EyeOff, Copy, Trash2, ShieldQuestion } from 'lucide-react';

const EnvTable = ({ envs, onReveal, onCopy, onDelete }) => {
    return (
            ...
            <table className="w-full text-left">
                <thead>
                    <tr className="bg-transparent border-b border-border">
                        <th className="px-6 py-4 text-xs font-bold text-text-secondary uppercase tracking-widest">Key</th>
                        ...
                        <tr>
                            <td colSpan="4" className="px-6 py-20 text-center">
                                <div className="flex flex-col items-center gap-3 text-text-muted">
                                    ...
                                    <p className="font-medium text-text-secondary">No environment variables found in this section.</p>
                                </div>
                            </td>
                        </tr>
                    ) : (
                        envs.map((env) => (
                            <tr key={env._id} className="hover:bg-[#1E293B] transition-colors group">
                                ...
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-2 group/value">
                                        <span className="font-mono text-sm text-text-primary">
                                            {env.revealed ? env.actualValue : env.value}
                                        </span>
                                        <button
                                ...
                                <td className="px-6 py-5">
                                    <p className="text-sm text-text-secondary line-clamp-1 max-w-xs">{env.description || '-'}</p>
                                </td>
                                <td className="px-6 py-5 text-right">
                                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={() => onCopy(env.revealed ? env.actualValue : env.value)}
