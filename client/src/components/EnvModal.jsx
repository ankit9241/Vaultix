...
import { X, Plus, Upload, FileText, Eye, EyeOff } from "lucide-react";
import { parseEnvContent } from "../utils/envUtils";
import Button from "./ui/Button";
import Modal from "./ui/Modal";
import CodeBlock from "./ui/CodeBlock";

const EnvModal = ({ isOpen, onClose, sectionId, onEnvCreated }) => {
  ...
  const resetForm = () => {
    setManualEnv({ key: "", value: "", description: "" });
    setEnvUploadText("");
    setParsedEnvs([]);
  ...
  const handleManualSubmit = async (e) => {
    e.preventDefault();
    if (!manualEnv.key || !manualEnv.value) {
    ...
    try {
      // Create single environment variable
      const newEnv = {
        ...manualEnv,
        revealed: false,
        actualValue: manualEnv.value,
      };
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                value={manualEnv.key}
                onChange={(e) =>
                  setManualEnv({ ...manualEnv, key: e.target.value })
                }
                placeholder="e.g. DATABASE_URL"
                className="w-full bg-code text-text-primary border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary focus:border-transparent font-mono placeholder:text-text-muted"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="password"
                required
                value={manualEnv.value}
                onChange={(e) =>
                  setManualEnv({ ...manualEnv, value: e.target.value })
                }
            ...
            <div>
              ...
              <textarea
                rows="3"
                value={manualEnv.description}
                onChange={(e) =>
                  setManualEnv({ ...manualEnv, description: e.target.value })
                }
                ...
                {uploadMethod === "paste" && (
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-code text-text-primary border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary focus:border-transparent font-mono text-sm placeholder:text-text-muted"
                    />
